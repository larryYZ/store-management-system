...
public class OrdersController implements Initializable {
	...
	public void displayUserOrders(User user) {
		ArrayList<Order> orders = new ArrayList<>();
		
		OrderDatabase orderDatabase = new OrderDatabase();
		orderDatabase.load();
		Map<String, ArrayList<Order>> allOrders = orderDatabase.orders;
		if (allOrders.containsKey(user.getName())) {
			orders = allOrders.get(user.getName());
		}
		...
		ordersListView.setOnMouseClicked(new EventHandler<MouseEvent>() {
	        ...
	        public void handle(MouseEvent event) {
	            ...
	            if (selectedOrder != null) {
	            	PageUtils.changeToOrderDetailsPage(event, user, user, selectedOrder);
	            }
	        }
	    });
	}
